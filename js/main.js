    ...
    function applyTranslation() {
        ...
        document.querySelectorAll('.card-link[data-link-type], .ach-link[data-link-type]').forEach(link => {
            const type = link.getAttribute('data-link-type');
            const keyMap = { code: 'projects.code', view: 'projects.view', download: 'projects.download' };
            if (keyMap[type]) {
                const codeHost = link.getAttribute('data-code-host');
                const hostLabel = type === 'code' && codeHost ? ` ${codeHost}` : '';
                let textNode = null;
                link.childNodes.forEach(node => {
                    if (node.nodeType === Node.TEXT_NODE && node.textContent.trim()) textNode = node;
                });
                if (textNode) {
                    textNode.textContent = ' ' + getTranslation(keyMap[type]) + hostLabel;
                } else {
                    Array.from(link.childNodes).forEach(node => {
                        if (node.nodeType === Node.TEXT_NODE) node.remove();
                    });

                    link.appendChild(document.createTextNode(' ' + getTranslation(keyMap[type]) + hostLabel));
                }
            }
        });
...
function renderExperience() {
    ...
    projects.forEach(proj => {
        ...
        if (proj.links?.gitlab) {
            const a = createElement('a', { href: proj.links.gitlab, target: '_blank',
                class: 'ach-link', 'data-link-type': 'code', 'data-code-host': 'GitLab', title: 'GitLab' });
            a.appendChild(createElement('i', { class: 'fab fa-gitlab' }));
            const text = getTranslation('projects.code');
            const span = createElement('span', { 'data-i18n': 'projects.code' }, [' ' + text + ' GitLab']);
            a.appendChild(span);
            linksEl.appendChild(a);
        }
        if (proj.links?.github) {
            const a = createElement('a', { href: proj.links.github, target: '_blank',
                class: 'ach-link', 'data-link-type': 'code', 'data-code-host': 'GitHub', title: 'GitHub' });
            a.appendChild(createElement('i', { class: 'fab fa-github' }));
            const text = getTranslation('projects.code');
            const span = createElement('span', { 'data-i18n': 'projects.code' }, [' ' + text + ' GitHub']);
            a.appendChild(span);
            linksEl.appendChild(a);
        }
        if (proj.links?.play) {
    ...
    function renderProjects(projects) {
        ...
        const projectCards = projects.map(project => {
            ...
            if (project.links?.gitlab) {
                const a = createElement('a', { href: project.links.gitlab, target: '_blank', rel: 'noopener noreferrer', class: 'card-link', 'data-link-type': 'code', 'data-code-host': 'GitLab', title: 'GitLab' });
                a.appendChild(createElement('i', { class: 'fab fa-gitlab' }));
                a.appendChild(document.createTextNode(' ' + getTranslation('projects.code') + ' GitLab'));
                linksContainer.appendChild(a);
            }
            if (project.links?.github) {
                const a = createElement('a', { href: project.links.github, target: '_blank', rel: 'noopener noreferrer', class: 'card-link', 'data-link-type': 'code', 'data-code-host': 'GitHub', title: 'GitHub' });
                a.appendChild(createElement('i', { class: 'fab fa-github' }));
                a.appendChild(document.createTextNode(' ' + getTranslation('projects.code') + ' GitHub'));
                linksContainer.appendChild(a);
            }
